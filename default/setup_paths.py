...
def setup_paths(_C, ensure_paths=True):
    ...
    _c.preproc_log = osp.join(_c.root_dir, 'data_log.yaml')
    ...
    if _C.mode == "design":
        ...
        # make dataset splits
        _C.built.splits = CN()
        _C.built.splits.dir_template = osp.join(_C.built.root_dir, "split_{}_{}") #subset key, room_nb, rotation(x30/y30), translation(x10,x-10)
        _C.built.splits.dirs = []
        _C.built.splits.template = CN()
        _C.built.splits.template.split_pcd = "pcd.las"
        _C.built.splits.template.split_pem = "pem.csv"
        _C.built.splits.template.split_feat = "features.csv"
        _C.built.splits.template.split_spg = "spg.h5"
        _C.built.splits.template.split_viz = "my_graph.ply"
        _C.built.splits.template.split_graph = "my_graph.graphml"
        _C.built.splits.template.preproc_log = "data_log.yaml"
        _C.built.splits.current = CN()
        _C.built.splits.current.pcd = ""
        _C.built.splits.current.pem = ""
        _C.built.splits.current.feat = ""
        _C.built.splits.current.spg = ""
        _C.built.splits.current.viz = ""
        _C.built.splits.current.graph = ""
        _C.built.splits.current.preproc_log = ""
...
def get_split_files(cfg, split_dir):

    dirs_dict = {}

    dirs_dict["pcd"] = osp.join(split_dir, cfg.splits.template.split_pcd)
    dirs_dict["spg"] = osp.join(split_dir, cfg.splits.template.split_spg)
    dirs_dict["pem"] = osp.join(split_dir, cfg.splits.template.split_pem)
    dirs_dict["viz"] = osp.join(split_dir, cfg.splits.template.split_viz)
    dirs_dict["graph"] = osp.join(split_dir, cfg.splits.template.split_graph)
    dirs_dict["feat"] = osp.join(split_dir, cfg.splits.template.split_feat)
    dirs_dict["preproc_log"] = osp.join(split_dir, cfg.splits.template.preproc_log)
    dirs_dict["instance_geom_dir"] = osp.join(split_dir, "instance_geometry")
    cfg = update_config_value(cfg, ["splits.current", dirs_dict])
